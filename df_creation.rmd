...
lapply(c("glue", "tidyverse", "openxlsx", 
         ...
         "pheatmap", "kableExtra"), require, character.only=TRUE)
```
...
common_df_patient <- common_df %>% 
  ...
  mutate(PTNA = case_when(
    str_detect(PTN, "LYMPHOMA", negate = FALSE) ~ "Lymphomas",
    str_detect(PTN, "ACUTE", negate = FALSE) ~ "Acute leukaemia",
    str_detect(PTN, "CHRONIC", negate = FALSE) ~ "Chronic leukaemia",
    TRUE ~ "Other"
  ) %>% as.factor) %>% 
  select(-c(LLTC, PTC, PTN, PSOCC, ICD, TUTERM, LLTN))   # Удалим ряд классификаций нозологии (оставим только PTCN)
```
...
```{r}
log_reg <- glm(ALIVE ~ PTNA + TRTYPE + CONDTYPE + RELAPYN + PTSTAT + chronic_GT, common_df_treatment_chronic, family=binomial)
summary(log_reg)
```
