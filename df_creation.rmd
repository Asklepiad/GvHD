...
```{r}
catStat_group <- function(df, factorr, grouperr){
  df %>% 
...
factor_table_group <- lapply(common_df_treatment_chronic %>% 
         select(where(is.factor) & !STERRES) %>% 
         colnames, 
       function(x) catStat_group(common_df_treatment_chronic, x, STERRES)) %>% 
       do.call(rbind, .) %>% 
  arrange(STERRES)
...
factor_table <- lapply(common_df_treatment_chronic %>% 
         select(where(is.factor)) %>% 
         colnames, 
       function(x) catStat(common_df_treatment_chronic, x) %>% 
       do.call(rbind, .))


do.call(rbind, factor_table)
factor_table[[3]]

ft1 <- common_df_treatment_chronic %>% 
    select(variant = "SEX") %>% 
    mutate(variant = as.character(variant) %>%  replace_na("no_data") %>% as.factor()) %>% 
    count(variant) %>% 
    rename(number = n) %>% 
...
ft2 <- common_df_treatment_chronic %>% 
    select(variant = "TUTERM") %>% 
    mutate(variant = as.character(variant) %>%  replace_na("no_data") %>% as.factor()) %>% 
    count(variant) %>% 
    rename(number = n) %>% 
    mutate(proportionIntoGroup = round(number/sum(number),3),
...
lapply(c(ft1, ft2), function(y) do.call(rbind, y))
