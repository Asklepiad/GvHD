...
```{r}
ref_binaric %>% 
  drop_na() -> ref_binaric1
Boruta(REF~.,ref_binaric1, ntree = 100, maxRuns = 30) %>% 
  TentativeRoughFix() -> boruta_trained1

boruta_trained1 %>% 
  attStats() %>% 
  rownames_to_column("Переменная") %>% 
  ggplot(aes(y = Переменная, x = meanImp, color = decision)) +
  geom_point() +
  geom_errorbar(aes(xmin = minImp, xmax = maxImp, width = 0.1)) +
  xlab('Среднее значение энтропии') +
  labs(color = 'Значимость переменной') +
  scale_color_discrete(labels = c('Подтвержденная','Неподтвержденная'))
```
